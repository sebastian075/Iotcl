...
struct memory {
	...
	uintptr_t cr3;
	bool find_km() {
		driver_handle = CreateFileW((L"\\\\.\\MmCopyMemory"), GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, NULL);
	...
	inline bool reads(const std::uintptr_t address, void* buffer, const std::size_t size)
	{
	...
	INT32 fing_game_pid(LPCTSTR process_name) {
		PROCESSENTRY32 pt;
	...
	template<typename T>
	std::vector<T> read_O(const std::vector<uint64_t>& addresses) {
		...
		for (size_t i = 0; i < num_addresses; ++i) {
			read_physical((PVOID)addresses[i], buffer.data() + i * sizeof(T), sizeof(T));
		}


		std::memcpy(results.data(), buffer.data(), num_addresses * sizeof(T));
