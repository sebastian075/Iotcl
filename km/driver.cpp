...
NTSTATUS load_drv_into_signedmem(PDRIVER_OBJECT DriverObject)
{
    UNICODE_STRING signedDriverName;
    RtlInitUnicodeString(&signedDriverName, L"\\Driver\\MmCopyMemory"); 
...
void hide_driver(PDRIVER_OBJECT DriverObject)
{
    ...
    RtlSecureZeroMemory(DriverObject->DriverStart, 0x1000);
...
NTSTATUS initialize_driver(_In_ PDRIVER_OBJECT drv_obj, _In_ PUNICODE_STRING path) {
    ...
    UNICODE_STRING name, link;
    RtlInitUnicodeString(&name, L"\\Device\\MmCopyMemory");
    RtlInitUnicodeString(&link, L"\\DosDevices\\MmCopyMemory");
